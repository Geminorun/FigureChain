...
该命令默认监听 `127.0.0.1:8000`，等价于
`uvicorn figure_chain.app:create_app --factory --host 127.0.0.1 --port 8000`。如需调整：

```powershell
uv run --no-sync figure-data run-api --host 0.0.0.0 --port 8000 --reload
```
...
启动 RQ worker（仅当 `.env` 设置 `FIGURE_AI_QUEUE_BACKEND=rq` 时需要）：
